...
export const useMainStore: StoreDefinition = defineStore("main", () => {
  ...
  const hints: Ref<NumberIndexedObject> = ref({
    ...
    2: [
      ...
      "match 쿼리는 검색어에 대해 분석과정을 수행해 텍스트를 토큰화한 뒤 검색에 사용합니다.",
    ...
    3: [
      "Term 쿼리는 정확한 값을 필드에서 찾아 문서를 검색합니다.",
      "정확한 단어나 숫자, 날짜 등을 찾고자 할 때 사용되며, 분석 과정을 거치지 않고 필드에 정확히 일치하는 값이 있는 문서를 찾습니다.",
      "(중요) Elasticsearch의 문자열 필드는 keyword와 text 타입으로 나뉩니다. text 타입은 토큰화되어 저장되고, keyword 타입은 그대로 저장됩니다",
      "(중요) 'Eddie Underwood'가 text로 저장될 때는 'Eddie'와 'Underwood'로 나뉘어 저장됩니다.",
      "(중요) Elasticsearch 역색인으로 작동합니다. 'Eddie Underwood'로 검색하였을 경우 text 타입으로 저장되었을 경우 역색인에 'Eddie Underwood'이 없습니다.",
      "(중요) 역색인에 존재하는 데이터는 'Eddie'와 'Underwood'만이 있습니다",
      "(중요) 일반적으로 text 타입으로 지정하는 필드는 keyword라는 하위필드가 존재합니다. .keyword로 접근가능합니다.",
      "<i>customer_full_name.keyword를 이용하여 customer_full_name 필드가 Eddie Underwood인 문서를 검색하세요.</i>"
    ],
    4: [
      "Range 쿼리는 지정된 범위 내의 값들을 가진 문서를 찾는 데 사용됩니다.",
    ...
    5: [
      ...
      "Boolean 쿼리의 하위 필드들은 항상 배열형태의 value만 받습니다.",
      "must는 포함해야 하는 조건, must_not은 제외해야 하는 조건을 정의합니다.",
      "must 쿼리는 지정된 조건을 만족하는 문서만 검색 결과에 포함됩니다.",
      "must_not 쿼리는 지정된 조건을 만족하지 않는 문서만 검색 결과에 포함됩니다.",
      "<i>customer_full_name 필드에서 'Mary'가 포함되어 있으면서 'Bailey'는 포함되지 않는 문서를 검색하세요.</i>"
    ],
    6: [
      "filter 쿼리는 검색 결과를 필터링하여 특정 기준을 만족하는 문서만을 결과로 반환합니다.",
      ...
      "(중요) filter 쿼리의 결과는 점수 계산이 생략된 만큼 must 쿼리보다 빠르며 캐싱이 적용됩니다.",
    ...
    10: [
      "Wildcard 쿼리를 사용하면 패턴 매칭을 통해 검색을 수행할 수 있습니다.",
      "예를 들어, 텍스트의 일부만 알고 있을 때, '*'나 '?'와 같은 와일드카드 문자를 사용하여 검색할 수 있습니다.",
